...
def get_prompt_extract_experiences() -> str:
    """提取简历/素材中的课外经历"""
    return """
    【任务】从提供的简历或文书素材文本中智能提取学生的所有课外经历。

    【重要说明】
    1. 你必须仔细分析文本，尽一切可能识别出所有课外经历，即使信息不完整也要提取。
    2. 特别注意中文简历的常见格式，包括但不限于：
       - "公司名称｜职位" 格式（如：云南悦鑫香料有限公司｜实验室助理）
       - "公司名称 - 职位" 格式
       - "公司名称，职位" 格式
       - "时间 + 公司/组织 + 职位" 格式（如：2023.06-2023.09 XX公司 数据分析实习生）
       - 项目式描述（如：参与XX项目，负责...）
    3. 常见时间格式：2023.06-2023.09、2023年6月-2023年9月、2023.06-至今、2023-2024等。
    ...
    组织: 云南悦鑫香料有限公司
    职位: 实验室助理
    职责: 协助实验室日常运营，参与香料成分分析实验，负责数据记录和整理
    成就: 完成了3个香料样品的关键成分分析，优化了实验数据记录流程
    技能: 实验室操作、数据分析、化学分析、实验报告撰写
    ========================

    【关键指令】:
    1. 即使信息不完整，也要提取尽可能多的信息
    2. 不要遗漏任何看起来像经历的描述
    3. 保持原文本中的具体细节
    4. 只提取事实，不添加分析或评论
    5. 如果无法确定某些字段，用"未知"或留空
    6. 输出必须是纯文本，不要使用Markdown
    """
